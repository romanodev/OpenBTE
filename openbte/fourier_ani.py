...
def get_decomposed_directions(ll,rot,argv):

    normal = argv['mesh']['face_normals'][ll,0:self.dim]
    dist   = argv['mesh']['dists'][ll,0:self.dim]
...
def assemble_fourier(kappa,argv):
  ...
  if not 'cache' in argv.keys():
    ...
    for ll in mesh['active_sides']:
      ...
      if not i == j:
       ...
       v_orth,_ = get_decomposed_directions(ll,kappa,mesh):
...
def solve_fourier(kappa_vec,**argv):
    ...
    if comm.rank == 0:
     ...
     old_kappa = 0
     C = np.zeros(argv_vec['n_elems'])
     for m,kappa in enumerate(kappa_vec):


         kappaf,tf[m,:],tfg[m,:,:],C = fourier(m,kappa,argv)
