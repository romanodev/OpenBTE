...
def solve_multiscale(geometry,material,first_guess,options_solve_rta)->'solver':
    ...
    def get_isothermal(temperature):

     RHS_isothermal = np.zeros((n_serial,len(rr),n_elems))
     tmp = np.einsum('rj,jn->rn',material['VMFP'][rr],geometry['dfixed'],optimize=True)  
     np.add.at(D.T,geometry['efixed'],tmp.clip(min=0).T)
     np.add.at(RHS_isothermal.T,geometry['efixed'],-np.einsum('us,mus->sum',tmp.clip(max=0),temperature))

     return RHS_isothermal 
    ...
    #form coefficients--
    #NEW-------------
    normals    = geometry['dfixed']/np.linalg.norm(geometry['dfixed'],axis=0)
    A          = np.einsum('mqj,js->mqs',material['sigma'],normals,optimize=True)
    Ap         = A.clip(min=0)
    sum_Ap     = Ap.sum(axis=0).sum(axis=0)
    tot        = np.divide(1, sum_Ap, out=np.zeros_like(sum_Ap), where=sum_Ap!=0)
    wiso       = Ap*tot #this is used for getting the average difference in temperature---
    kappa_bulk = 1e-9*np.einsum('is,mui,m,uj,js->s'  ,normals,material['sigma'],mfp,material['VMFP'],normals)
    tmp        = 1e-9*np.einsum('nvs,m,uj,js->smu'   ,Ap,mfp,material['VMFP'],normals)
    coeff      = 2*np.einsum('smu,s->smu',tmp,1/kappa_bulk)
    #----------------
    ...
    B_res = geometry['generation'][np.newaxis,np.newaxis,:]+np.einsum('m,qc->mqc',mfp,P)
    ...
    def residual(m,n,q,x):
      ...
      return mfp[m]*np.einsum('c,c->c',D[n],x) + mfp[m]*utils.sparse_dense_product(i,j,Gm[n],x) + x - geometry['generation']-mfp[m]*(P[n]+ RHS_isothermal[m,n])

    DeltaT_old = np.zeros_like(DeltaT)
    Boundary_old = np.zeros((len(rr),len(geometry['eb'])))
    TISO  = np.zeros(len(geometry['efixed']))
    while kk <max_bte_iter and error > max_bte_error:
        ...
        #outgoing_iso
        TISOp  = np.zeros(len(geometry['efixed']))

        correction  = np.zeros((n_serial,n_parallel,len(geometry['efixed'])))

        #correction  = -np.einsum('smu,s->mus',coeff,TISO)

        temp = geometry['fixed_temperature'][np.newaxis,np.newaxis,:] + correction

        RHS_isothermal  = get_isothermal(temp)

        for n,q in enumerate(rr):
           
           for m in range(n_serial):
                 #----------------------------------------
                 #B = DeltaT +  mfp[m]*(P[n] + get_boundary(Boundary[n]) + RHS_isothermal[n] + mfp[m]*correction[n]) + geometry['generation'] 
                 B = DeltaT +  mfp[m]*(P[n] + get_boundary(Boundary[n]) + RHS_isothermal[m,n]) + geometry['generation']
                 ...
                 #Collect outgoing temperatures--  
                 #if len(geometry['efixed']) > 0:
                 #np.add.at(TISOp,np.arange(geometry['efixed'].shape[0]),(X[geometry['efixed']]-geometry['fixed_temperature'])*Ap[m,n])
                 np.add.at(TISOp,np.arange(geometry['efixed'].shape[0]),(X[geometry['efixed']]-geometry['fixed_temperature'])*wiso[m,n])
                 #np.add.at(TISOp,np.arange(geometry['efixed'].shape[0]),X[geometry['efixed']]*wiso[m,n])
                 #np.add.at(TISOp,np.arange(geometry['efixed'].shape[0]),geometry['fixed_temperature']*wiso[m,n])
                 #np.add.at(TISOp,np.arange(geometry['efixed'].shape[0]),geometry['fixed_temperature']*wiso[m,n])
        ...
        comm.Allreduce([TISOp,MPI.DOUBLE],[TISO,MPI.DOUBLE],op=MPI.SUM)

        #print(TISO/geometry['fixed_temperature'])
        ...
        #options = {'verbose':False,'temp_error':DeltaT - DeltaT_old,'DSA':False,'G':G}
        #output  = fourier(material,geometry,kappa,**options)
        ...
        #DeltaT  += output['Temperature_Fourier']
        #---------------
