...
class Material(object):
 ...
 def compute_isotropic_2DSym(self,**argv):  
   ...
   #experimental---
   polar_plus = np.array([np.sin(phi + Dphi/2.0),np.cos(phi + Dphi/2.0),np.ones(n_phi)]).T
   polar_minus = np.array([np.sin(phi - Dphi/2.0),np.cos(phi - Dphi/2.0),np.ones(n_phi)]).T
   #-------------
   ...
   #experimental---
   direction_ave2 = np.einsum('ij,kj->ikj',azimuthal_ave,polar)/Dphi
   direction_ave_plus = np.einsum('ij,kj->ikj',azimuthal_ave,polar_plus)/Dphi
   direction_ave_minus = np.einsum('ij,kj->ikj',azimuthal_ave,polar_minus)/Dphi
   #-----------------------------
   ...
   kappa_directional2 = np.zeros((n_mfp*n_phi,3)) 
   kappa_directional_plus = np.zeros((n_mfp*n_phi,3)) 
   kappa_directional_minus = np.zeros((n_mfp*n_phi,3)) 
   norm = 0
   norm_tb = 0
   #test = np.zeros(n_mfp*n_phi)
   
   test = 0
   for p in range(n_phi): 
    for t in range(n_theta): 
     for m in range(n_mfp_bulk):
      #(m1,a1,m2,a2) = self.get_linear_indexes(mfp,mfp_bulk[m]*ftheta[t]*np.sin(theta[t]),scale='log',extent='True')
   ...
   #repeat angle---
   polar_ave = np.array([np.repeat(polar_ave[:,i],n_mfp) for i in range(3)]).T
   polar_plus = np.array([np.repeat(polar_plus[:,i],n_mfp) for i in range(3)]).T
   polar_minus = np.array([np.repeat(polar_minus[:,i],n_mfp) for i in range(3)]).T
   ...
   return {'CollisionMatrix':A,\
           'TCOEFF':TCOEFF,\
           'angle_plus':polar_plus,\
           'angle_minus':polar_minus,\
           'temp_vec':temp_vec,\
           'angle_map':angle_map,\
           'dphi':Dphi,\
           'kappa_directional':kappa_directional,\
           ...
           'control_angle':polar_ave,\
           'control_angle_new':polar,\
           'factor':[np.sin(Dphi/2),np.cos(Dphi/2)/Dphi],
           'kappa_bulk_tot':np.sum(kappa_bulk)}


 def compute_isotropic_2DSym_new(self,**argv):  
   ...
   polar_ave = polar*fphi
   #direction_ave = direction*ftheta
   ...
   #kappa_directional2 = np.zeros(n_mfp*n_phi) 
   #kappa_directional_plus = np.zeros((n_mfp*n_phi,3)) 
   #kappa_directional_minus = np.zeros((n_mfp*n_phi,3)) 
   norm = 0
   norm_tb = 0
   ...
   # print(kappa_directional[n][0]/kappa_directional2[n]/polar[n][0])
