...
def Plot(**argv):
   ...
   if model == 'maps':
     ...
     vtu(material,solver,geometry,argv)
      ...
      if comm.rank == 0:
        save(argv.setdefault('suppression_file','suppression'),{'mfp':material['mfp_sampled'],'suppression':s['Sm']})
      ...
      if dim == 2:
        output = kappa_mode_2DSym(material,solver,argv)
      else:
        output = kappa_mode_3D(material,solver)

      if argv.setdefault('save',True) and comm.rank == 0:
        save(argv.setdefault('kappa_mode_file','kappa_mode'),output)
