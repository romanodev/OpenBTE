...
#from .solve_rta2 import *
#from .solve_multiscale import *
import pkg_resources  
#from .first_guess import first_guess
from .fourier import solve_fourier
import warnings
...
def print_bulk_info(mat,mesh):
           ...
           #dirr = ['x','y','z']
           #print(colored('  Applied Thermal Gradient :         ','green')+ mesh['applied_gradient'],flush=True)
...
def Solver(**argv):
        ...
        #kappa   = np.repeat([material['kappa']],len(geometry['elems']),axis=0)
        #output  = solve_fourier(material,geometry,kappa,**options)
        
        #Solve bte--
        if not only_fourier:
           output = solve_rta(geometry,material,argv)
           #output = solve_rta2(geometry,material,argv)
           #output_rta = solve_multiscale(geometry,material,output,argv)
