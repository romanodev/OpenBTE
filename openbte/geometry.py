...
class Geometry(object):
 ...
 def compute_connecting_matrix(self):
   ...
   A  = dok_matrix((self.nle,self.nle), dtype=np.float64)
   for ll in self.side_list['active'] :
 ...
 def compute_connecting_matrix_new(self):
   ...
   nc = len(self.elems)
   #N  = sparse.DOK((nc,nc,3), dtype=np.float64)

   ns = len(self.elem_side_map[0])
   #CP  = sparse.DOK((nc,ns,3), dtype=np.float64) #this is a elem X n_side map of normal
   #CM  = sparse.DOK((nc,ns,3), dtype=np.float64) #this is a elem X n_side map of normal

   #N_new = sparse.DOK((nc,nc,3), dtype=np.float64)
   ...
   for ll in self.side_list['active'] :
     ...
     l1 = elems[0]
     l2 = elems[1]
     vol1 = self.get_elem_volume(l1)
     vol2 = self.get_elem_volume(l2)
