...
class Material(object):
 ...
 def __init__(self,**argv):
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
   ...
   self.output = {}   
   if argv.setdefault('model','nongray') == 'full':
     self.compute_full_bte(**argv)
   else:
 ...
 def compute_mat_2D(self,argv):  
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
   ...
   #J1 = np.outer(kappa_bulk/mfp_bulk,ftheta*dtheta*np.sin(theta)).flatten()/sum(kappa_bulk/mfp_bulk)   
   #J2 = np.outer(kappa_bulk/pow(mfp_bulk,2),dtheta).flatten()/sum(kappa_bulk/pow(mfp_bulk,2))   #x2 (symmetry) and /2 (average)
   #J3 = np.outer(1/mfp_bulk,ftheta*dtheta*np.sin(theta)).flatten()
