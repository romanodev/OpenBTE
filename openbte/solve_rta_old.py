...
def solve_rta(**argv):
    ...
    kappaf,kappafp = np.zeros((2,argv['n_serial'],argv['n_parallel']))
    ...
    while kk < argv['max_bte_iter'] and error > argv['max_bte_error']:
        ...
        for n,q in enumerate(argv['rr']):
           ...
           #Get ballistic/Fourier kappa
           if argv['multiscale']:
              kappa_fourier_m =  np.einsum('c,mc->m',mesh['kappa_mask'],tf-DeltaT) - np.einsum('c,m,i,mci->m',mesh['kappa_mask'],mfp,mat['VMFP'][q,0:argv['dim']],tfg)
              #kappafp[:,q] =  np.einsum('c,mc->m',mesh['kappa_mask'],tf-DeltaT) - np.einsum('c,m,i,mci->m',mesh['kappa_mask'],mfp,mat['VMFP'][q,0:argv['dim']],tfg)
              #kappafp[:,q] = np.einsum('c,mc->m',mesh['kappa_mask'],tf-DeltaT)  - np.einsum('c,m,i,mci->m',mesh['kappa_mask'],mfp,mat['VMFP'][q,0:argv['dim']]+np.array([1e-8,1e-8]),tfg)
              kappafp[:,q] = np.einsum('c,mc->m',mesh['kappa_mask'],tf-DeltaT)  - np.einsum('c,m,i,mci->m',mesh['kappa_mask'],mfp,mat['VMFP'][q,0:argv['dim']],tfg)
              #kappafp[:,q] =  np.einsum('c,mc->m',mesh['kappa_mask'],tf-DeltaT) - np.einsum('c,m->m',mesh['kappa_mask'],mfp)*mat['VMFP'][q,0]*0.1
    ...
    output =  {'kappa':kappa_vec,'temperature':DeltaT,'flux':J,'kappa_mode':kappa,'kappa_mode_f':kappaf}#,'suppression':Sup}
