...
def compute_boundary_condition_data(data,**argv):
    ...
    #Apply gradient---------------------------------------------
    applied_gradient = argv.setdefault('applied_gradient',[1,0])
    applied_gradient.append(0)
    #-----------------------------------------------------------

    #Add jump to periodic boundary conditions---
    delta = 1e-2
    ...
    for kl,ll in enumerate(tmp) :
     ...
     tmp = np.dot(normal,applied_gradient)
     if abs(tmp) > 0: 
      if tmp < - delta :
         side_value[ll] = -np.linalg.norm(1)
      if tmp > delta :
         side_value[ll] =  np.linalg.norm(1)
    #------------------    
    ...
    #Compute area flux--
    if dim == 2 and applied_gradient[0] == 1:
       area_flux = data['size'][1]
       length    = data['size'][0]
    elif dim == 3 and applied_gradient[0] == 1:
       area_flux = data['size'][1]*data['size'][2]
       length    = data['size'][0]
    elif dim == 2 and applied_gradient[1] == 1:
       area_flux = data['size'][0]
       length    = data['size'][1]
    #elif dim == 3 and applied_gradient[1] == 1:
    #   area_flux = data['size'][0]*data['size'][2]
    #elif direction == :
    #   area_flux = data['size'][0]*data['size'][1]
    elif np.linalg.norm(applied_gradient) == 0:
       area_flux = 0 
       length    = 1
    else:
    ...
    cm = np.zeros(n_elems)
    cp = np.zeros(n_elems)
    perturbation = np.zeros(n_elems)
    ss = 1
    for ll in list(data['periodic_sides']):
     ...
     if abs(tmp) > 0:
       ...
       cp[e1]      = data['areas'][ll]
       cm[e2]      = data['areas'][ll]
       perturbation[e2] = ss*data['areas'][ll]


    #From thermalizing sides-----------------------
    for ll,dt in zip(*(data['fixed_temperature_sides'],data['fixed_temperature'])):
     e1,e2 = side_elem_map[ll]
     normal = data['face_normals'][ll]  
     ...
     if tmp > 0:
       ...
       total_area += data['areas'][ll]
       cp[e1] = data['areas'][ll]
       perturbation[e2] = dt*data['areas'][ll]
    #----------------------------------------------

       
    data['periodic_side_values'] = np.array([periodic_side_values[ll]  for ll in data['periodic_sides']])
    data['pp'] = np.array(pp)
    data['applied_gradient'] = np.array(applied_gradient)
    data['flux_sides'] = np.array(flux_sides)


    #ss: it is negative if the normal is [-1,0,0]. It depends on the mesh
    if area_flux == 0: area_flux = 1
    normalization = area_flux #total_area
    data['kappa_factor'] = np.array([-length*ss])
    data['cm'] = cm/area_flux
    data['cp'] = cp/area_flux
    data['perturbation'] = perturbation/area_flux
...
def compute_connecting_matrix(data):
   ...
   #Boundary sides----
   #sb = a_sides[np.isin(a_sides,b_sides)]
   sb = np.asarray(b_sides,dtype=int)
   eb = side_elem_map[sb][:,0]
   db = face_normals[sb,:dim].T*side_areas[sb]/elem_volumes[eb]
   #-------------

   #Isothermal sides----
   sfixed  = data['fixed_temperature_sides']
...
def import_mesh(**argv):
   ...
   side_list['fixed_temperature_sides'] = np.asarray(fixed_temperature_sides,dtype=int)



   for side in side_list['Boundary'] + list(side_list['fixed_temperature_sides']) :   
   ...
   data.update({'fixed_temperature':np.asarray(fixed_temperature)})
   data.update({'fixed_temperature_sides':np.asarray(fixed_temperature_sides,dtype=int)})
...
def compute_data(data,**argv):
   ...
   data['meta'] = np.asarray([len(data['elems']),0,data['dim'],len(data['nodes']),len(data['active_sides'])],np.float64)
