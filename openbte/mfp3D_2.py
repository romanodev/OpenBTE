...
def generate_mfp3D_2(**argv): 
   ...
   comm.Allreduce([tcp,MPI.DOUBLE],[tc,MPI.DOUBLE],op=MPI.SUM)
     ...
     # kappa_directional[m1,index] += a1 * g2[m]*dirr[t,p]
     # kappa_directional[m2,index] += a2 * g2[m]*dirr[t,p]
     # temp_coeff[m1,index] += a1 * g1[m]*domega[t,p]*sym
     # temp_coeff[m2,index] += a2 * g1[m]*domega[t,p]*sym
