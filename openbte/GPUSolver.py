   ...
   if self.mode =='cpu': 
    ...
    return x
      ...
      for i in range(n_batch.value):
        Acsr = sp.csr_matrix((self.A[i,:],(self.row,self.col)),shape=(n.value,m.value),dtype=self.tt)
        global_data +=list(Acsr.sorted_indices().data)
      ...
      B = np.ascontiguousarray(B.flatten(),dtype=self.tt)
      

      b = gpuarray.to_gpu(B.astype(self.tt))

      dx = pycuda.gpuarray.empty_like(b,dtype=self.tt)
