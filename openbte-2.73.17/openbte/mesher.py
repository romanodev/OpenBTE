...
import os
import time
from openbte.helpers import *


def generate_hydro(**argv):
...
Point(3) = {-L/2,   Ly/2,0,h};
Point(4) = { L/2,   Ly/2,0,h};
Point(5) = { Lx/2,  Ly/2,0,h};
Point(6) = { Lx/2, -Ly/2,0,h};
Point(7) = { L/2,  -Ly/2,0,h};
Point(8) = {-L/2,  -Ly/2,0,h};
Line(1) = {1,2};
Line(2) = {2,3};
Line(3) = {3,4};
Line(4) = {4,5};
...
Line(9) = {3,8};
Line(10) = {4,7};
Line Loop(1001) = {1,2,9,8};
Plane Surface(1)= {1001};
Line Loop(1002) = {9,-7,-10,-3};
Plane Surface(2)= {1002};
Line Loop(1003) = {4,5,6,-10};
...
Physical Line("ISO_0.5") = {3};
Physical Line("ISO_-0.5") = {7};
Physical Line("Boundary") = {1,2,4,5,6,8};

'''


  strc = strc.replace('STEP',str(argv['step']))
  strc = strc.replace('XX',str(argv['Lx']))
  strc = strc.replace('YY',str(argv['Ly']))
  strc = strc.replace('LL',str(argv['L']))
...
def generate_bulk_2D(argv):
  ...
  if argv['boundary'][1] == 'Periodic':
   ...
   if applied_gradient[1]==1:
    strc = r'''Physical Line('ISO_-0.5') = {3};''' + '\n'
    store.write(strc)
    strc = r'''Physical Line('ISO_0.5') = {1};''' +'\n'
    store.write(strc)
   else:
    bs.append(1)
    bs.append(3)
  ...
  if argv['boundary'][0] =='Periodic':
   ...
   if applied_gradient[0]==1:
    strc = r'''Physical Line('ISO_0.5') = {2};''' + '\n'
    store.write(strc)
    strc = r'''Physical Line('ISO_-0.5') = {4};''' +'\n'
    store.write(strc)
   else:
    bs.append(2)
    bs.append(4)
...
class Mesher(object):
 ...
 def __init__(self,argv):
  ...
  if model == 'film_in_plane':
     apply_film_in_plane(argv) 

  if model == 'film_cross_plane':
     apply_film_cross_plane(argv) 

  if model == 'square_lattice':
     argv = square_lattice(argv) 

  if model == 'hexagonal_lattice':
     argv = hexagonal_lattice(argv) 

  if model == 'honeycomb_lattice':
     argv = honeycomb_lattice(argv) 
