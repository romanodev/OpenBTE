...
def suppression(solver,material,options_suppression)->'suppression':
 ...
 if comm.rank == 0:
  ...
  if options_suppression.setdefault('show',False):
   fig = go.Figure(data=go.Scatter(x=mfp_sampled, y=Sm))
   fig.update_xaxes(title_text="Mean Free Path [m]", type="log")
   fig.update_yaxes(title_text="Suppression Function",range=[0,1.2])
   fig.show()
